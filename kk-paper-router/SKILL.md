...
description: >-
  ...
  润色时强制逐段拆句、逐句执行润色，逐句出改前/改后/改前翻译/改后翻译/为什么改
  五段句卡对照，超过两段落盘为 md 文件并转带封面的 PDF。审稿时并行
...
| 用袁老师风格写 / 改 / 中翻英 / 审阅把关挑毛病 / 回复审稿（提到袁老师、袁非牛、yfn） | `yfnskills` 整条接管：接稿问诊后按其内部四条工作流走（整篇初稿、润色、中翻英、rebuttal），交付 md+PDF 成对。**语句与写作规则用 yfnskills 自己那套**（不注入 11 条，冲突时以 yfnskills 为准），**但润色交付形制与 story 一致、缺一不可**：导师口吻意见书 + 逐句五段句卡对照（改前/改后/改前翻译/改后翻译/为什么改，全覆盖不合并）+ 完整改后终稿 final（`revised/` tex+pdf）。**交付层规则一律以本路由为准**（目录 `yfn_<venue>_<时间戳>/`、落盘即转带封面 PDF、复用 `paper_context`、rebuttal 先读 `paper_reviews/`），事实性红线两边一致 |
...
| 润色 / 中译英 / 去 AI 味 / 缩写扩写 | 读 [references/polish-mode.md](references/polish-mode.md)。**给了整篇论文就默认全文润色，不问范围**。三级拆分：先把全文切成几个部分、每部分拆段落、每段拆句子，逐句执行，逐句出改前/改后/改前翻译/改后翻译/为什么改五段句卡对照；部分数 ≥5 时按部分并行派子 agent，父进程按原顺序拼成总版、通读做合并复审再转带封面 PDF。点名袁老师风格的转 `yfnskills` |
...
2. `research-paper-writing`：逐章节写故事，一段一个意思，claim 对齐实验。
   ...
   不堆并列、包装不超证据），当写作约束用，不出逐句句卡。
3. **故事定了就自动接润色模式**，不是写完停下来问要不要润。初稿阶段仍不做
   句级句卡，避免故事没定就抠字；三层一完，立刻按 polish-mode 对这篇初稿
   全文逐句出五段句卡对照并转带封面 PDF，中间不停。根据已有论文写初稿也走这条，
...
- **规则不因入口而变**：同一件事，单独点名调用和被路由联动调用，执行规则完全
  一致——五段句卡契约、逐句全覆盖、命名带 venue 加时间戳、落盘即转带封面 PDF、
  复用 `paper_context`、开跑后不停，六条一视同仁。**唯一按入口变的是「用谁的语句
...
- 稿内 `XX` 与 `AUTHOR_INPUT_NEEDED` 按证据缺失处理，任何环节不得编造
  数据、引用或官方政策。
- 审稿与写作不混在同一轮。
- **输出语言分工**：投的是英文论文，但作者只读中文，所以两类内容分开——
  **讲解、意见、诊断、理由、报告叙述、进度汇报、给用户的话一律中文**；
  **要落进英文稿的部分必须纯英文**：论文标题、摘要、正文、图注、表题、公式说明、
  逐句句卡的「改前」和「改后」、完整改后终稿、投英文刊的 rebuttal 回复信正文，
  一字都不许夹中文，不中英混排，不在英文句里塞中文括注，中文标点不进英文句。
  句卡里「改前」「改后」纯英文，「改前翻译」「改后翻译」「为什么改」用中文。判断依据很简单：
  这段文字将来会不会原样出现在投出去的稿子里——会，就纯英文；只是讲给作者听的，
  就中文。yfnskills 也照此，它本来就是英文改写稿用英文、解释用中文。
...
- **技能互通**：各 skill 不是孤岛，执行中需要其他能力时直接调对应 skill。
  ...
  polish-mode 的五段句卡契约与 11 条风格规则；`paper-version-compare` 选完主线
...
- **自治工作流的注入规则**：`yfnskills` 与 `kkstoryline` 都是完整流水线，
  ...
  - `yfnskills` **不注入**：它自带 80 条规则白名单、按章节量化的语言画像、
    ...
    **润色交付形制也一律照 story 那套**：不管 story 还是 yfnskills，润色都要出
    逐句五段句卡对照（改前/改后/改前翻译/改后翻译/为什么改，全覆盖不合并、不省略）
    加一份应用了全部「改后」的完整改后终稿（final）。story 的 final 是
    `final_paper` 三格式，yfnskills 的 final 是 `revised/` 的 tex+pdf。逐句句卡用
    哪套语句判据按各自的写作规则（story 用 11 条，yfnskills 用它自己那套），
    但「逐句五段句卡 + final」这个形制两边都不许省。分头改完后都要通读整篇做合并复审连贯性。
