---
name: kkimage-visio-ppt
description: 把图片重建成可编辑的 Office 图形。适用：论文架构图/流程图/示意图转成可编辑 PPT 图形、复现别人论文里的图、截图变可编辑形状、1:1 还原图片、图片转 VBA、图片转 PowerPoint/Excel/Word 形状。中文触发词：图转PPT、图片转可编辑、把图做成可编辑的、复现这张图、重画这张架构图、图转VBA。只做"已有图片转可编辑图形"，从数据画实验曲线走 scientific-visualization，从零设计示意图走 baoyu-diagram。Use when users want kkimage-visio-ppt academic image-to-vba reconstruction: convert academic figures, scientific diagrams, slides, screenshots, or other images into vba, office drawing code, powerpoint/excel/word shapes, editable shape reconstruction, 1:1 recreation, pixel-like approximation, or hybrid editable Office shape reconstruction.
---

# KKImage Visio PPT

> **属于 kk-paper-skills 系统**：被直接点名进入时，**先完整读一遍
> `kk-paper-router/SKILL.md` 的全局铁律再开跑**，这是闸门不是提醒。三条附加约束：
>
> 1. **重建自己论文里的图时先查档案**：`paper_context/<稿件名>/` 里有该图的图注、
>    模块名、数据流向记录时以档案和基准稿为准，不靠看图猜。图里的模块名与正文
>    不一致，是一致性问题，报给用户，别在图上自作主张改名。
> 2. **产物落 `figures_<venue或主题>_<YYYYMMDD-HHMM>/`**：`.bas`、`.pptx`/`.pptm`、
>    裁图资产、运行报告都进同一个目录。VBA 与 PPTX 本身是成品格式，不转 PDF；
>    运行报告和自检报告是 md，落盘验收后立即转同名 PDF。
> 3. **画图类分工**：本 skill 只做「已有图片 → 可编辑 Office 图形」。从数据画
>    实验曲线与消融图走 `scientific-visualization`；从零设计架构示意图走
>    `baoyu-diagram`；整篇论文做汇报片子走 `nature-paper2ppt`，它需要其中某张图
>    变可编辑时再回头调本 skill。
>
> 本仓库内部不许跳过路由；只有完全没装 `kk-paper-router` 的独立部署，才按本文件
> 自治执行。此段属本地增补，升级上游包后要加回。

kkimage-visio-ppt academic image-to-vba reconstruction skill. The workflow focuses on editable Office Shapes first, with carefully labeled raster crops only when complex scientific artwork should be preserved.
