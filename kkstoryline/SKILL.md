...
## 九、专业 PDF 排版标准（md2pdf）

**总原则：交付 PDF 要像精制投稿包，不是 md 直接怼过去。** 参考质量标杆是斜切封面 + 分节线 + 高亮 callout + 深蓝表头斑马表 + 圆角框箭头流程图 + 五段句卡这种观感。要点：
- **别裸转**：合订本/总册/意见书等重要交付用下面的组件排版，能配色的地方配色、该画结构图就画。
- **配色不锁死**：默认深蓝 `#22384f` + 橙 `#c0472f`，觉得不搭就在 frontmatter 写 `primary: #xxxxxx` / `accent: #xxxxxx` 换，别被某一套色限制。
- **防乱码**：中英混排统一走 md2pdf（已配好系统 CJK 字体栈），别手拼 HTML 漏字体导致方块字；换机先 `python md2pdf.py --check`。
- **可独立精制**：md2pdf 组件不够用时（复杂结构图/对照示意图），允许自己写带样式的 HTML 再转 PDF，只要达到上面的观感、不乱码即可。

`md2pdf.py` 内置这些组件，够用就别造轮子：

**A. 封面 frontmatter。** md 顶部写 `--- ... ---`，渲染成斜切双色封面（kicker + 大标题 + 副标题 + 右上旋转印章 + 底部信息行），正文页右上角带运行页眉。键（都可选，有 `title` 才出封面；`primary`/`accent` 可换配色）：
...
**B. 五段句卡（`::: card`）。** 逐句润色每一句用一张卡片，渲染成带标题条、五个彩色标签字段的圆角卡片，比横向表格清楚：
...
`**改前** **改后**` 纯英文；`**改前翻译** **改后翻译** **为什么改**` 用中文。`::: card` 之间空一行。

**C. 高亮 callout（`::: note 标题`）。** 权威规则、关键提醒、核心判断放进 callout 框，一眼抓住重点：

```
::: note 权威规则
本清单是后续改稿的唯一执行依据，冲突以此为准。
:::
```

**D. 流程/结构图（`::: flow`）。** 阶段路线、依赖关系画成横向圆角框 + 箭头，一行一个节点 `标题 | 副标题`，自动换行：
...
**E. 表格自动上色。** 普通 markdown 表格会自动加深色表头 + 斑马行，关键决定/对照清单直接写表即可。
