...
5. 流水线覆盖条款（优先于标准卡原文）：
   ...
   e) 【仅第 5 步逐句润色】句子层面的质量标准照 kk-paper-router 技能的
      `references/polish-mode.md`「KK 语句风格追加要求」11 条执行。该文件与本
      skill 装在同一个 skills 目录下，**父进程派发时必须先定位它并把绝对路径
      写进本条**（找不到就把 11 条原文直接抄进 prompt，不许让子 agent 自己找）。
