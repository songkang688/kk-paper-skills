...
polish-mode 的五段句卡契约逐句给出（`::: card` 围栏）：改前 | 改后 | 改前翻译 |
改后翻译 | 为什么改，改前/改后纯英文、三项中文说明用中文，无需修改的句子也出卡片并
写明它为何成立，禁止「同上」「略」。**protocol 第三节允许对比表只挑关键差异，那是
针对版本对比表；最终润色版一律逐句五段句卡，两者不冲突。
